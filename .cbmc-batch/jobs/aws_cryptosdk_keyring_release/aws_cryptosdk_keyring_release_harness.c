...
void destroy(struct aws_cryptosdk_keyring *keyring) {
    assert(keyring != NULL);
    assert(AWS_OBJECT_PTR_IS_WRITABLE(keyring));
    free(keyring);
}
...
void aws_cryptosdk_keyring_release_harness() {
    ...
    if (keyring != NULL) {
        ...
        __CPROVER_assume(aws_cryptosdk_keyring_vt_is_valid(keyring->vtable));
        __CPROVER_assume(aws_atomic_load_int(&keyring->refcount) > 0);
    }
